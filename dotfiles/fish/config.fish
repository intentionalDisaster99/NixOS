...
alias windows-nosave="sudo grub-reboot 1 && sudo reboot"
alias windows="sudo grub-reboot 1 && sudo mkdir -p /run/systemd/sleep.conf.d && printf '[Sleep]\nHibernateMode=reboot\n' | sudo tee /run/systemd/sleep.conf.d/windows-reboot.conf > /dev/null && systemctl hibernate && sudo rm /run/systemd/sleep.conf.d/windows-reboot.conf"
# alias windows="sudo grub-reboot 1 && echo reboot | sudo tee /sys/power/disk && echo disk | sudo tee /sys/power/state"
